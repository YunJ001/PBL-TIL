...
import type { Lion, LionFormData, RandomUserResponse } from '../types/lion';

interface AddFormProps {
  onAdd: (lion: Omit<Lion, 'id'>) => void;
  onCancel: () => void;
}

const INITIAL_FORM: LionFormData = {
  name: '', part: 'Frontend', skills: '', intro: '',
  bio: '', email: '', phone: '', website: '', motto: '',
};

function AddForm({ onAdd, onCancel }: AddFormProps) {
  const [form, setForm] = useState<LionFormData>(INITIAL_FORM);
    ...
    <div className="add-form-container">
      <div className="form-grid">
        ...
        <div className="form-group form-full">
          ...
          <input type="text" id="input-motto" name="motto" value={form.motto} onChange={handleChange} placeholder="예: 데이터를 바꾸면 화면도 바뀐다!" />
        </div>
      </div>
